...
namespace DIPLOM
{
    public partial class FormPairsDif2 : Form
    {
        FormPairsDif1 pairs1 = new FormPairsDif1();
        String currentUser;
        String idUser;
        int Win = 5;
        int Time = 50;
        ...
        public FormPairsDif2()
        {
            InitializeComponent();
            mem2.AssignPictures(timerCountdown, pictures, panelCardsHolder);
        }
        ...
        public void carryPoints(int Points2)
        {
            Oldpoints = Points2;
            this.labelPoints.Text = Convert.ToString(Points2);
        }
        private void GivePictures(PictureBox Box)
        {
            mem2.GivePictures(Box, timerFlip, timerCountdown, labelWin, labelPoints, label1, buttonAgain, buttonNext, Win, Points, currentUser, idUser);
        }
        private void timerFlip_Tick(object sender, EventArgs e)
        {
            mem2.timerFlip_Tick(timerFlip);
        }

        private void timerCountdown_Tick(object sender, EventArgs e)
        {
            mem2.timerCountdown_Tick(label1, labelLose, timerCountdown, buttonAgain, panelCardsHolder);
        }

        private void buttonAgain_Click(object sender, EventArgs e)
        {
            mem2.buttonAgain_Click(labelWin, labelLose, label1, labelPoints, buttonAgain, buttonNext, timerCountdown, panelCardsHolder, Time, Oldpoints);
        }
        private void buttonNext_Click(object sender, EventArgs e)
        {
            Points = Convert.ToInt32(labelPoints.Text);
            FormPairsDif3 pairs3 = new FormPairsDif3();
            pairs3.DesktopLocation = this.DesktopLocation;
            pairs3.SetUser(currentUser, idUser);
            pairs3.Show();
            pairs3.carryPoints(Points);
            this.Hide();
        }
        #region picBoxes
        ...
        private void FormPairsDif2_MouseMove(object sender, MouseEventArgs e)
        {
            move.Move(sender, e, this);
        }

        private void FormPairsDif2_MouseDown(object sender, MouseEventArgs e)
        {
